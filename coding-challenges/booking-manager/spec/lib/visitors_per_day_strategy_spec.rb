describe VisitorsPerDayStrategy do
  subject(:strategy) { described_class.new(limit) }
  let(:limit) { 2 }

  describe '#evaluate' do
    let(:booking) { BookingRequest.new(Time.now, 600, 'dave', 'rico') }
      ...
      context 'with multiple visits across multiple days' do
        before do
          Timecop.freeze(Date.today - 2) do
            strategy.evaluate(BookingRequest.new(Time.now, 600, 'john', 'rico'))
            strategy.evaluate(BookingRequest.new(Time.now, 600, 'dave', 'rico'))
          end
